...
function SetlistView({ setlists, songs, onPerformanceMode }) {
    ...
    return (
      <div className="flex-1 flex items-center justify-center">
        <div className="text-center">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Setlist not found</h3>
          <p className="mt-1 text-sm text-gray-500">The requested setlist could not be found.</p>
          <Link to="/setlists" className="mt-4 inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-primary-700 bg-primary-100 hover:bg-primary-200">
            Back to Setlists
          </Link>
        </div>
      </div>
  ...
  const setlistSongs = setlist.songs
    .map(songTitle => {
      ...
      const songEntry = Object.entries(songs).find(([id, song]) => 
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      <div className="p-6">
        <Link to="/setlists" className="inline-flex items-center text-sm text-gray-500 hover:text-gray-700 mb-4">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{setlist.name}</h1>
        ...
        <div className="space-y-4">
          {setlistSongs.length > 0 ? (
            ...
            <div className="text-center py-8">
              <p className="text-gray-500">No matching songs found in this setlist.</p>
            </div>
          )}
        </div>
      </div>
